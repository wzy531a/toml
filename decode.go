...
func verify(data interface{}, rv reflect.Value, ignore_fields []string) error {
	...
	// laziness
	if k >= reflect.Int && k <= reflect.Uint64 {
		return verifyInt(data, rv)
	}
	switch k {
	case reflect.Struct:
		return verifyStruct(data, rv)
	case reflect.Map:
		return verifyMap(data, rv)
	case reflect.Slice:
		return verifySlice(data, rv)
	case reflect.String:
		return verifyString(data, rv)
	case reflect.Bool:
		return verifyBool(data, rv)
	case reflect.Interface:
		// we only support empty interfaces.
		if rv.NumMethod() > 0 {
			e("Unsupported type '%s'.", rv.Kind())
	...
	case reflect.Float32:
		fallthrough
	case reflect.Float64:
		return verifyFloat64(data, rv)
	}
	return e("Unsupported type '%s'.", rv.Kind())
}

func verifyStruct(mapping interface{}, rv reflect.Value) error {
	tmap, ok := mapping.(map[string]interface{})
	if !ok {
		return mismatch(rv, "map", mapping)
	}
...
func verifyMap(mapping interface{}, rv reflect.Value) error {
	tmap, ok := mapping.(map[string]interface{})
	if !ok {
		return badtype("map", mapping)
	}
...
func verifySlice(data interface{}, rv reflect.Value) error {
	slice, ok := data.([]interface{})
	if !ok {
		return badtype("slice", data)
	}
...
func verifyDatetime(data interface{}, rv reflect.Value) error {
	if _, ok := data.(time.Time); ok {
		return nil
	}
	return badtype("time.Time", data)
...
func verifyString(data interface{}, rv reflect.Value) error {
...
func verifyFloat64(data interface{}, rv reflect.Value) error {
...
func verifyInt(data interface{}, rv reflect.Value) error {
...
func verifyBool(data interface{}, rv reflect.Value) error {
...
func verifyAnything(data interface{}, rv reflect.Value) error {
	return nil
}
